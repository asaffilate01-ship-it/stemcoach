...
export function PersistentCoach() {
  ...
  return (
    <button
      ...
      aria-label={t("coach.askNamed", { name: preferredCoach.name })}
      title={t("coach.askNamed", { name: preferredCoach.name })}
    >
      ...
      <span className="hidden min-w-0 sm:block">
        <span className="block truncate text-[10px] font-bold uppercase tracking-wider text-primary">{t("coach.persistent")}</span>
        <span className="flex items-center gap-1 text-xs font-semibold">
          <MessageCircle className="h-3 w-3" /> {t("coach.askNamed", { name: preferredCoach.name })}
        </span>
      </span>
    </button>
  );
