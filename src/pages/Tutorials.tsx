  ...
  return <div className="min-h-screen bg-background">
    ...
    <main className="container mx-auto max-w-5xl px-4 py-8">
      <div className="mb-8">
        ...
        <p className="mt-3 text-sm font-medium text-primary">{completed.length} of {tutorials.length} tutorials mastered on this device</p>
      </div>
        ...
        {filtered.map((tutorial) => {
          ...
          return <article key={tutorial.id} className="overflow-hidden rounded-2xl border bg-card shadow-sm">
            <button onClick={() => setOpenId(open ? null : tutorial.id)} className="flex w-full items-center gap-4 p-5 text-left">
              ...
              <div className="flex-1"><p className="text-xs font-semibold uppercase tracking-wide text-primary">{subjectNames[tutorial.subject]} · {tutorial.level}</p><h2 className="mt-1 flex items-center gap-2 text-lg font-bold">{tutorial.title}{isComplete && <CheckCircle2 className="h-4 w-4 text-emerald-500" />}</h2><p className="mt-1 text-sm text-muted-foreground">{tutorial.summary}</p></div>
            ...
            {open && <div className="space-y-6 border-t px-5 py-6">
              ...
              <Button variant="outline" className="gap-2" onClick={() => navigate(`/ai-tutor?subject=${encodeURIComponent(tutorial.subject)}`)}><img src={mascot.image} alt="" className="h-6 w-6 rounded-lg object-cover"/><MessageCircle className="h-4 w-4"/>Ask {mascot.name} about this lesson</Button>
              <section><h3 className="mb-3 font-semibold">Knowledge checks</h3><div className="space-y-5">{checkpoints.map((checkpoint, checkpointIndex) => {
                ...
                }}>{option}</Button>)}</div>{checkpointChoice && <p className={`mt-3 rounded-lg p-3 text-sm ${checkpointChoice === checkpoint.answer ? "bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "bg-amber-500/10 text-amber-800 dark:text-amber-300"}`}>{checkpointChoice === checkpoint.answer ? "Correct. " : `Not quite. The answer is ${checkpoint.answer}. `}{checkpoint.explanation}</p>}</div>;
              })}</div></section>
            </div>}
          </article>;
        })}
