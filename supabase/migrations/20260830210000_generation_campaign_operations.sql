...
-- Queue updates must never accidentally restart a paused or cancelled campaign.
...
-- Workers only claim jobs from active campaigns. Legacy queue rows without a
-- campaign remain processable for backwards compatibility.
...
-- Atomic lifecycle actions used by the protected batch-generate Edge Function.
...
-- Enrich the existing aggregate endpoint without returning millions of rows.
