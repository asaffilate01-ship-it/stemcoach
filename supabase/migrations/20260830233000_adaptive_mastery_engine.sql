...
BEGIN
  ...
  WHERE candidate.topic_rank <= CASE WHEN safe_mode = 'diagnostic' THEN 2 ELSE 4 END
  ORDER BY candidate.priority, candidate.recently_seen, candidate.topic_rank, random()
  LIMIT safe_count;
END;
$$;
...
CREATE OR REPLACE FUNCTION public.get_learner_mastery_dashboard(_curriculum text DEFAULT NULL)
RETURNS TABLE (
  curriculum text,
  subject text,
  topic text,
