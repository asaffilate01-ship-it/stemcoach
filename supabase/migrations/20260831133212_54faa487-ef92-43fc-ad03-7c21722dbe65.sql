...
AS $$
  SELECT
    ...
    (SELECT count(*)::integer FROM public.class_learning_path_items item WHERE item.path_id = path.id) AS tutorial_count,
    (
      SELECT count(*)::integer
      FROM public.class_learning_path_items item
      JOIN public.user_tutorial_progress progress
        ON progress.tutorial_id = item.tutorial_id
       AND progress.user_id = member.user_id
       AND progress.completed_at IS NOT NULL
      WHERE item.path_id = path.id
    ) AS completed_count,
    (
      SELECT max(coalesce(progress.completed_at, progress.last_opened_at))
      FROM public.class_learning_path_items item
      JOIN public.user_tutorial_progress progress
        ON progress.tutorial_id = item.tutorial_id
       AND progress.user_id = member.user_id
    ...
    (
      (SELECT count(*) FROM public.class_learning_path_items item WHERE item.path_id = path.id) > 0
      AND (SELECT count(*) FROM public.class_learning_path_items item WHERE item.path_id = path.id) = (
        SELECT count(*)
        FROM public.class_learning_path_items item
        JOIN public.user_tutorial_progress progress
          ON progress.tutorial_id = item.tutorial_id
  ...
  JOIN public.classes classroom ON classroom.id = path.class_id
  JOIN public.class_members member ON member.class_id = path.class_id
  LEFT JOIN public.profiles profile ON profile.id = member.user_id
  WHERE classroom.teacher_id = auth.uid()
$$;

REVOKE ALL ON FUNCTION public.create_class_learning_path(uuid, text, text, timestamptz, text[]) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.create_class_learning_path(uuid, text, text, timestamptz, text[]) TO authenticated;
REVOKE ALL ON FUNCTION public.get_teacher_learning_path_progress() FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.get_teacher_learning_path_progress() TO authenticated;